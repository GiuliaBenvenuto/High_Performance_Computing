...
int main()
{
  ...
  float *temp1_init, *temp2_init;
  float *temp1_ref, *temp2_ref, *temp1, *temp2, *temp_tmp, *temp1_mod, *temp2_mod;  
  // double *temp1_mod, *temp2_mod, *temp_tmpmod;
  const long int size = ni * nj * sizeof(float);
  ...
  if ((ni*nj) > MAX){
    ...
    int iter=0;
    long int size_temp = (ni*nj) * sizeof(float);
    int ni_new = ni;
    int nj_new = nj;
    for (iter=0; size_temp<MAX; iter++){
        nj_new = nj_new / 2;
        ni_new = ni_new / 2;
        size_temp = ni_new * nj_new * sizeof(float);
    }
    printf("How many iterations: %d \n", iter);

    dim3 threads(256); //(16, 16);
    dim3 dimblock(((ni_new-ni_new/2)*(nj_new-nj_new/2)+ threads.x-1)/threads.x);
    ...
    temp1_ref = (float*)malloc(size_temp);
    temp2_ref = (float*)malloc(size_temp);
    temp1 = (float*)malloc(size_temp);
    temp2 = (float*)malloc(size_temp);

    temp1_init = (float*)malloc(size_temp);
    temp2_init = (float*)malloc(size_temp);

    cudaMalloc(&temp1_mod, size_temp);
    cudaMalloc(&temp2_mod, size_temp);
    ...
    for( int i = 0; i < ni_new*nj_new; ++i) {
      temp1_init[i] = temp1_ref[i];
      temp2_init[i] = temp1_ref[i];
    }

    cudaMemcpy(temp1_mod, temp1_init,  size_temp, cudaMemcpyHostToDevice);
    cudaMemcpy(temp2_mod, temp2_init,  size_temp, cudaMemcpyHostToDevice);
    ...
    for (istep=0; istep < nstep; istep++) {
      ...
      step_kernel_mod<<<dimblock, threads>>>(ni_new, nj_new, tfac, temp1_mod, temp2_mod);
  ...
  cudaMemcpy(temp1_init, temp1_mod, size_temp, cudaMemcpyDeviceToHost);
  cudaMemcpy(temp2_init, temp2_mod, size_temp, cudaMemcpyDeviceToHost);
