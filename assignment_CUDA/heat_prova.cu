// SOLUTION
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

// Simple define to index into a 1D array from 2D space
#define I2D(num, c, r) ((r)*(num)+(c))

/*
 * `step_kernel_mod` is currently a direct copy of the CPU reference solution
 * `step_kernel_ref` below. Accelerate it to run as a CUDA kernel.
 */


/*
abbiamo una riglia lineare con blocchi lineari. 
abbiamo tanti thread tanti quanti sono gli elementi interni alla cornice
dobbiamo andare a prendere elementi nel centro della matrice (quelli ai bordi sono esclusi)

For example, if you have a matrix with n rows and m columns, and you want to exclude a border of size k from the top, 
bottom, left, and right sides of the matrix, you can use the following formula to calculate the number of cells in the matrix
excluding the border:   (n - 2 * k) * (m - 2 * k)

abbiamo utilizzato cudamallocmanaged almeno creiamo un puntatore accessibile sia da cpu che gpu (ci risparmia malloc e copy)
 

*/

__global__ void step_kernel_mod(int ni, int nj, float fact, float* temp_in, float* temp_out)
{
  int i00, im10, ip10, i0m1, i0p1;
  float d2tdx2, d2tdy2;

  int index = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = blockDim.x * gridDim.x;

  // loop over all points in domain (except boundary)
  for (int idx = index; idx < (nj-2)*(ni-2); idx += stride ) {
    
    int i = idx % (ni-2) + 1;
    int j = idx / (ni-2) + 1;
    // find indices into linear memory
    // for central point and neighbours
    i00 = I2D(ni, i, j);
    im10 = I2D(ni, i-1, j);
    ip10 = I2D(ni, i+1, j);
    i0m1 = I2D(ni, i, j-1);
    i0p1 = I2D(ni, i, j+1);

    // evaluate derivatives
    d2tdx2 = temp_in[im10]-2*temp_in[i00]+temp_in[ip10];
    d2tdy2 = temp_in[i0m1]-2*temp_in[i00]+temp_in[i0p1];

    // update temperatures
    temp_out[i00] = temp_in[i00]+fact*(d2tdx2 + d2tdy2);
  }
}

void step_kernel_ref(int ni, int nj, float fact, float* temp_in, float* temp_out)
{
  int i00, im10, ip10, i0m1, i0p1;
  float d2tdx2, d2tdy2;


  // loop over all points in domain (except boundary)
  for ( int j=1; j < nj-1; j++ ) {
    for ( int i=1; i < ni-1; i++ ) {
      // find indices into linear memory
      // for central point and neighbours
      i00 = I2D(ni, i, j);
      im10 = I2D(ni, i-1, j);
      ip10 = I2D(ni, i+1, j);
      i0m1 = I2D(ni, i, j-1);
      i0p1 = I2D(ni, i, j+1);

      // evaluate derivatives
      d2tdx2 = temp_in[im10]-2*temp_in[i00]+temp_in[ip10];
      d2tdy2 = temp_in[i0m1]-2*temp_in[i00]+temp_in[i0p1];

      // update temperatures
      temp_out[i00] = temp_in[i00]+fact*(d2tdx2 + d2tdy2);
    }
  }
}

int main()
{
  int istep;
  int nstep = 200; // number of time steps
  
  // Specify our 2D dimensions
  const int ni = 100;
  const int nj = 100;
  
  float tfac = 8.418e-5; // thermal diffusivity of silver
  float *temp1_ref, *temp2_ref, *temp1, *temp2, *temp_tmp, *temp1_mod, *temp2_mod;
  const int size = ni * nj * sizeof(float);

  dim3 threads(32);
  //dim3 dimblock(((ni-2) * (nj-2) + threads.x - 1) / threads.x);  //dimensione blocchi proporzionale al problema
  dim3 dimblock(((ni-ni/2) * (nj-nj/2) + threads.x - 1) / threads.x);
  
  printf("dim3 threads: %u \n", threads.x);
  printf("dim3 block: %u \n", dimblock.x);
  printf("total num of threads: %u\n", threads.x*dimblock.x);
  printf("total num of pixels: %d\n", ni*nj);

  cudaEvent_t start_ref, stop_ref; 
  cudaEvent_t start_mod, stop_mod;
  cudaEventCreate(&start_ref);
  cudaEventCreate(&stop_ref);
  cudaEventCreate(&start_mod);
  cudaEventCreate(&stop_mod);

  temp1_ref = (float*)malloc(size);
  temp2_ref = (float*)malloc(size);
  //cudaMallocManaged(&temp1, size);
  //cudaMallocManaged(&temp2, size);
  temp1 = (float*)malloc(size);
  temp2 = (float*)malloc(size);



  cudaMalloc(&temp1_mod, size);
  cudaMalloc(&temp2_mod, size);


  // Initialize with random data
  for( int i = 0; i < ni*nj; ++i) {
    temp1_ref[i] = temp2_ref[i] = temp1[i] = temp2[i] = (float)rand()/(float)(RAND_MAX/100.0f);
  }
  cudaMemcpy(temp1_mod, temp1,  size, cudaMemcpyHostToDevice);
  cudaMemcpy(temp2_mod, temp2,  size, cudaMemcpyHostToDevice);

  
  cudaEventRecord(start_ref);
  // Execute the CPU-only reference version
  for (istep=0; istep < nstep; istep++) {
    step_kernel_ref(ni, nj, tfac, temp1_ref, temp2_ref);

    temp_tmp = temp1_ref; // swap the temperature pointers
    temp1_ref = temp2_ref;
    temp2_ref= temp_tmp;
  }

  cudaEventRecord(stop_ref);
  cudaEventSynchronize(stop_ref);

  float elapsed_ref = 0;
  cudaEventElapsedTime(&elapsed_ref, start_ref, stop_ref);
  printf("Elapsed ref time: %f microseconds\n", elapsed_ref) ;
  

  cudaEventRecord(start_mod);
  // Execute the modified GPU version using same data
  for (istep=0; istep < nstep; istep++) {
    step_kernel_mod<<<dimblock, threads>>>(ni, nj, tfac, temp1_mod, temp2_mod);
    cudaDeviceSynchronize();
    // swap the temperature pointers
    temp_tmp = temp1_mod;
    temp1_mod = temp2_mod;
    temp2_mod = temp_tmp;
  }


  cudaMemcpy(temp1, temp1_mod, size, cudaMemcpyDeviceToHost);
  cudaMemcpy(temp2, temp2_mod, size, cudaMemcpyDeviceToHost);

  cudaEventRecord(stop_mod);
  cudaEventSynchronize(stop_mod);

  float elapsed_mod = 0;
  cudaEventElapsedTime(&elapsed_mod, start_mod, stop_mod);
  printf("Elapsed mod time: %f microseconds\n", elapsed_mod) ;

  
  float maxError = 0;
  // Output should always be stored in the temp1 and temp1_ref at this point
  for( int i = 0; i < ni*nj; ++i ) {
    if (abs(temp1_mod[i]-temp1_ref[i]) > maxError) { maxError = abs(temp1_mod[i]-temp1_ref[i]); }
  }

  // Check and see if our maxError is greater than an error bound
  if (maxError > 0.0005f)
    printf("Problem! The Max Error of %.5f is NOT within acceptable bounds.\n", maxError);
  else
    printf("The Max Error of %.5f is within acceptable bounds.\n", maxError);
  

  cudaFree(temp1_mod);
  cudaFree(temp2_mod);

  free( temp1_ref );
  free( temp2_ref );
  free( temp1 );
  free( temp2 );



  return 0;
}